...
while error>.01,
    ...
    Model.nworkers_white=.7*Model.nworkers_white+.3*(nworkers_white_temp+sum(nworkers_exp_temp,3));
    Model.nworkers_blue=.7*Model.nworkers_blue+.3*nworkers_blue_temp;
    Model.nworkers_exp=.7*Model.nworkers_exp+.3*nworkers_exp_temp;
    wage_blue_old=wage_blue;
    [wage_blue,wage_white,wage_exp,quant_prod,nworkers_exp_upd,lab_demand_white] = Static_Eq(Model);
    error=sum(sum(abs(wage_blue_old-wage_blue)))
end
